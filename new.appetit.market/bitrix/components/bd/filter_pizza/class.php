...
class CBitrixCatalogSmartFilter extends CBitrixComponent
{
	...
	public function fillItemValues(&$resultItem, $arProperty, $flag = null)
	{
		...
		$file_id = null;
		$url_id = null;
		switch($arUserType['PROPERTY_TYPE'])
		{
		case "L":
	...
	*/
	...
	public function convertUrlToCheck($url)
	{
		...
		{
			$item = false;
			$smartPart = preg_split("/-(from|not|to|is|or)-/", $smartPart, -1, PREG_SPLIT_DELIM_CAPTURE);
			foreach ($smartPart as $i => $smartElement)
			{
				...
				}elseif ($smartElement === "not"){
					$valueId = $this->searchValue($item["VALUES"], $smartPart[$i+1]);

					$result[$item["VALUES"][$valueId]["CONTROL_NAME"]] = array('type'=>'E_','val'=>$valueId);
				}
			}
			unset($item);
		}
		...
		if ($apply)
		{
			...
			foreach($this->arResult["ITEMS"] as $PID => $arItem)
			{
				...
				else
				{
					...
					{
						if($_GET[$arItem["VALUES"][$key]['CONTROL_ID']]=='E'){
							$smartPart['not'][] = $ar["URL_ID"];
						}else{
							if (
								(
									$ar["CHECKED"]
	...
	public function encodeSmartParts($smartParts)
	{
		...
		foreach ($smartParts as &$smartPart)
		{
			...
			foreach ($smartPart as $i => $smartElement)
			{
				...
				elseif($i == 'not'){
					foreach ($smartElement as $el){
						$urlPart .= urlencode('-'.$i.'-'.$el);
					}
				}
				elseif ($i == 'from' || $i == 'to')
