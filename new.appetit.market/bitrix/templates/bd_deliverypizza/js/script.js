            ...
            $.ajax({
                ...
                success: function(data){
                    ...
                    $.each(data.products,function(i,item){
                        ...
                        animateNumbers($basket_item.find('.product-sum span').first(),item.LOCAL_SUM);
                    });
...
function checkInput() {
    $('.bd-input input,.bd-input textarea').each(function () {
        var $cont = $(this).parents('.bd-input');
        if($(this).val().trim().length>0){
            $cont.addClass('filled')
        }else{
...
function getProductDetail(id){
    ...
    $.ajax({
        ...
        data: {PRODUCT_ID: id},
        dataType: "json",
        url: '/include/ajax1/common.php?action=getDetail',
        success: function(data){
...
$(document).ready(function(){
    ...
    $('a[href="#callback-submit"]').click(function() {
        $('.alx_feedback_popup.order-callback').click();
        return false;
    });
...
function initSlider() {
    ...
    $('.responsive-slider').slick({
        ...
        slidesToScroll: 4,
        slide: 'div',
        prevArrow: '<button type="button" data-role="none" class="slick-prev" aria-label="Previous" tabindex="0" role="button"><svg viewBox="0 0 100 100"><path d="M 10,50 L 60,100 L 70,90 L 30,50  L 70,10 L 60,0 Z" class="arrow"></path></svg></button>',
        nextArrow: '<button type="button" data-role="none" class="slick-next" aria-label="Next" tabindex="0" role="button"><svg viewBox="0 0 100 100"><path d="M 10,50 L 60,100 L 70,90 L 30,50  L 70,10 L 60,0 Z" class="arrow" transform="translate(100, 100) rotate(180) "></path></svg></button>',
        responsive: [
            {
                breakpoint: 1200,
                settings: {
