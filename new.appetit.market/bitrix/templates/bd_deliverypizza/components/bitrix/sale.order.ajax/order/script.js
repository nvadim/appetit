...
$(document).ready(function () {
	var box =  $('#order-ajax-box');

    function reloadOrder() {
        var form = $('#bx-soa-order-form');
        var action = form.data('avar');
        var data = form.serialize()+'&'+action+'=processOrder&ajaxOrder=Y';
        var contain = $('#order-ajax-box');
        $.ajax({
            type: 'post',
            data: data,
    ...
    box.on('change', '.order-main-input', function () {
        reloadOrder();
    });

    box.on('click','.delivery-type-toggle a',function(e){
            e.preventDefault();
            var id = parseInt($(this).data('id'));
            $('.delivery-type-toggle a').removeClass('active');
            $(this).addClass('active');
            $('#delivery_order_input').val(id).change();
        });
    box.on('change','.delivery-time-type_ input',function(e){
        if($(this).val()==1){
            $('.delivery-date-time_').show();
        }else{
            $('.delivery-date-time_').hide();
            $('#time_value_inp').val($(this).val())
        }
    });
    box.on('change', '.delivery-date-time_ input, .delivery-date-time_ select', function () {
    	var data = '';
		$('.delivery-date-time_ input, .delivery-date-time_ select').each(function () {
            data +=$(this).val() + ' ';
        });
		$('#time_value_inp').val(data);

    });
    box.on('click', '.send-order', function () {
        var form = $('#bx-soa-order-form');
        var action = form.data('avar');
        var data = form.serialize()+'&'+action+'=saveOrderAjax&save=Y';
        $.ajax({
            type: 'post',
            data: data,
            dataType: 'json',
