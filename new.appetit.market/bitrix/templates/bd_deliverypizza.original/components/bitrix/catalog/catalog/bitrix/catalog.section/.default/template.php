...
<?if($arItem["PROPERTIES"]['BANNER']["VALUE"]):?>
	<div class="col-lg-3 col-xl-3 col-sm-6 product-ajax-cont <?php if(isset($_GET['AJAX_PAGE'])): ?>animate<?php endif; ?>">
		<div data-id="<?=$arItem['ID'];?>" class="product banner" id="<?=$this->GetEditAreaId($arItem['ID']);?>">
			<?
				$Banner =  CFile::ResizeImageGet($arItem["PROPERTIES"]['BANNER']["VALUE"], array('width'=>'255', 'height'=>'455'), BX_RESIZE_IMAGE_EXACT, true);
				$Banner2 = CFile::ResizeImageGet($arItem["PROPERTIES"]['BANNER']["VALUE"], array('width'=>'222', 'height'=>'490'), BX_RESIZE_IMAGE_EXACT, true);
				$Banner3 = CFile::ResizeImageGet($arItem["PROPERTIES"]['BANNER']["VALUE"], array('width'=>'344', 'height'=>'560'), BX_RESIZE_IMAGE_EXACT, true);
				$Banner4 = CFile::ResizeImageGet($arItem["PROPERTIES"]['BANNER']["VALUE"], array('width'=>'275', 'height'=>'510'), BX_RESIZE_IMAGE_EXACT, true);
				$Banner5 = CFile::ResizeImageGet($arItem["PROPERTIES"]['BANNER']["VALUE"], array('width'=>'344', 'height'=>'645'), BX_RESIZE_IMAGE_EXACT, true);
			?>
	...
	<div class="col-lg-3 col-xl-3 col-sm-6 product-ajax-cont <?php if(isset($_GET['AJAX_PAGE'])): ?>animate<?php endif; ?>">
		<div data-id="<?=$arItem['ID'];?>" itemscope itemtype="http://schema.org/Product" class="product" id="<?=$this->GetEditAreaId($arItem['ID']);?>"  data-unit="<?php echo $arItem['PROPERTIES']['UNITS']['VALUE']; ?>">
			<div data-modal="product-detail" class="preview md-trigger">
				...
				<?php if($arItem['PROPERTIES']['NO_SALE']['VALUE']=='Y'):?><div title="<?php echo GetMessage("item_no_sale_title"); ?>" class="without-sale"></div><?php endif; ?>
				...
				<div class="product-footer<?if($arItem["OLD_PRICE"]):?><?else:?> base-price<?endif;?>">
					<div itemprop="offers" itemscope itemtype="http://schema.org/Offer" class="product-prices font-fix">
	                  ...
	                  <span class="old-price" <?if(!$arItem["OLD_PRICE"]):?>style="display: none;" <?endif;?>>
	                  	<span class="line-through"><?=number_format($arItem["OLD_PRICE"],0 ,'.',' ');?></span><span class="currency"><?=CURRENCY_FONT; ?></span>
	                  </span>
						<span  class="current-price"><span itemprop="price"><?=number_format($arItem["PRICE"],0 ,'.',' ');?></span><span class="currency"><?=CURRENCY_FONT; ?></span>
					...
					<div class="product-actions <?php if(intval($arItem['PROPERTIES']['BUY_IT']['VALUE'])!==0 && $arItem['PROPERTIES']['BUY_IT']['VALUE']-$arItem['PROPERTIES']['BUY_IT_COMPLETE']['VALUE'] > 0): ?>with-progress<?php endif; ?> <?php if(intval($arItem['PROPERTIES']['BUY_IT']['VALUE'])!==0 &&  $arItem['PROPERTIES']['BUY_IT']['VALUE']-$arItem['PROPERTIES']['BUY_IT_COMPLETE']['VALUE'] <= 0): ?>progress-complete<?php endif; ?>">
						<?php $delta = Bd\Deliverypizza\Models\Product::GetProductDelta($arItem['ID']); if($delta<0) $delta = 0; ?>
